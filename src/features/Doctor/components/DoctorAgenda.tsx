...
import { useAuth } from '@/config/AuthProvider'; // Tu AuthProvider
...
const DoctorAgenda: React.FC = () => {
  const { user } = useAuth(); // Obtener usuario autenticado de Keycloak

  // Obtener keycloakId del doctor autenticado
  const doctorKeycloakId = user?.id; // Este es el keycloakId del token
  ...
  // Hook de servicios usando keycloakId
  const {
    appointments,
    availableSlots,
    patients,
    ...
    createAppointment,
    calendarReady
  } = useAppointmentService(doctorKeycloakId || ''); // ✅ Usar keycloakId

  // Verificar que el doctor esté autenticado
  if (!user || !doctorKeycloakId) {
    return (
  ...
  // Para debugging - ver el keycloakId
  useEffect(() => {
    console.log('🔍 Doctor keycloakId:', doctorKeycloakId);
    console.log('🔍 User completo:', user);
  }, [doctorKeycloakId, user]);

  // Crear objeto selectedDoctor con keycloakId
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validar que selectedDateString no sea undefined
    ...
    const appointmentData: CreateAppointmentRequest = {
      ...
      date: selectedDateString, // Ahora TypeScript sabe que no es undefined
      time: selectedTime,
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-900">
            Agenda - Dr. {selectedDoctor.name}
          </h1>
          <p className="text-sm text-gray-600">
            ID: {selectedDoctor.keycloakId}
          </p>
        </div>
                ...
                <div key={index} className="relative">
                  {date.isCurrentMonth ? (
                    <button
                      ...
                      className={`h-12 w-full text-sm font-medium relative rounded-lg transition-colors ${selectedDate === date.day
                ...
                <div className="grid grid-cols-3 gap-2">
                  {availableSlots.map((slot) => (
                    <button
                      ...
                      className={`h-10 text-sm font-medium rounded-lg border transition-colors ${selectedTime === slot.time
